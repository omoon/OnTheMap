...
class ParseClient: NSObject {
    ...
    func getStudentLocations(completionHander: (success: Bool, errorString: String?) -> Void) {
        ...
        let task = taskForGETMethod(Methods.StudentLocation, parameters: ["limit": 100, "order": "-updatedAt"]) { (result, error) in
            if error == nil {
    ...
    func postMyLocation(myUdacityUserData: UdacityUserData, mapItem: MKMapItem, urlString: String, completionHandler: (success: Bool, errorString: String?) -> Void) {
        ...
        if myLocations.count == 0 {
            let task = taskForPOSTMethod("/StudentLocation", parameters: [:], jsonBody: jsonBody) { (results, error) in
                if let error = error {
            ...
            let task = taskForPUTMethod("/StudentLocation/\(myLocations[0].objectID!)", parameters: [:], jsonBody: jsonBody) { (results, error) in
                if let error = error {
                    completionHandler(success: false, errorString: error.localizedDescription)
                } else {
                    self.getStudentLocations({ (success, errorString) in
